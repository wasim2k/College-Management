...
def teacher_session():
     while 1:
        ...
        if user_option =="1":
            ...
            command_handler.execute("SELECT username,date,status, status FROM attendance")
            records =command_handler.fetchall()
...
def admin_session():
    while 1:
        ...
        if user_option =="1":
            ...
            command_handler.execute("INSERT INTO users (username,password,privilige) VALUES (%s,%s,'student')",query_vals)
            db.commit()
            ...
            query_vals = (username,password)
            command_handler.execute("INSERT INTO users (username,password,privilige) VALUES (%s,%s,'teacher')",query_vals)
            db.commit()
            ...
            command_handler.execute("DELETE FROM users WHERE username = %s AND privilige =%s ",query_vals)
            db.commit()
...
def auth_teacher():
    ...
    query_vals =(username,password)
    command_handler.execute("SELECT * FROM users WHERE username = %s AND password =%s AND privilige ='teacher'",query_vals)
